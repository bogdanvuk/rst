...
ps = block("Procesorski Sistem", size=p(16,20), margin=p(0.5, 0.5), text_align="nw", dotted=True)()
...
mem_con = ps_comp("DDR3 Memorijski Kontroler").below(cpu)()

ddr3 = ps_comp("DDR3 Memorija").left(mem_con)()
...
pl = block("EFTIP hardverski koprocesor", size=p(24,34), margin=p(0.5, 0.5), text_align="nw", dotted=True).right(ps, 2)()

cu = comp("Kontrolna Jedinica").align_x(pl.r(1, 0)).align_y(cpu.p)()
bus([intercon.e(2), cu.w(2)], style='<->')()
t_axi = bus_text("AXI4").align(prev(1).pos(0.5), prev().s(0.5))()


inst_mem = comp("Memorija za Trening Skup").right(cu)()
bus([cu.e(0.5), inst_mem.w(0.5)], style='<->')()

fit_calc = comp("Kalkulator Preciznosti").right(inst_mem)()
...
block("Niz DT Memorija", dt_mem[2].s(1.0) - dt_mem[0].n() + (2,2), dotted=True, text_font='footnotesize', text_align='bc').align(dt_mem[0].n() - (1,1))()
...
block("Klasifikator", nte[2].s(1.0) - nte[0].n() + (2,2), dotted=True, text_font='footnotesize', text_align='bc').align(nte[0].n() - (1,1))()
