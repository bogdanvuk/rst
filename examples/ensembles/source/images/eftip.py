...
dt_mem += t_nte("$DTD_{D^M}$").right(dt_mem['$DTD_2'], 2)

pl += text(r"$\cdot\cdot\cdot$", font='\\footnotesize').align(mid(dt_mem['$DTD_2'].c(), dt_mem['$DTD_{D^M}'].c()), prev().c())
pl += text("DT Memory Array", font='\\footnotesize').align(mid(dt_mem['$DTD_2'].c(), dt_mem['$DTD_{D^M}'].c()), prev().n(0.5, -0.7))
...
nte += t_nte("$TLE_{D^M}$").below(dt_mem[2]).aligny(nte[-1].p)
