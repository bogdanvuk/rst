...
The NTE operation is again pipelined internally for the maximal throughput. Furthermore, there is no additional overhead that could come from the need of pipeline flushing, since for this application of the DT accuracy calculation, the instances enter the pipeline in predefined order, one by one, until whole dataset is classified. Also, the initial latency compared to
