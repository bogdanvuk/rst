...
bus_text("Instance Input").align_y(prev(1)[1], prev().s()).align_x(evaluator.p)()
...
# bus_text("threshold").align(mem_intf.e(4) + (1,0), prev().s())()
# bus_text("child data: left ID, right ID").align(mem_intf.e(4) + (1,0), prev().n())()

path([mem_intf.w(2) - (8, 0), mem_intf.w(2)], style='->')()
bus_text("Child ID Input").align_y(prev(1)[0], prev().s()).align_x(evaluator.p)()
...
bus_text("Leaf ID Input").align_y(prev(1)[0], prev().s()).align_x(evaluator.p)()

comp = block("$\geq$", p(4,4), text_font="Large").right(add[-1], 6).align_y(add[-1].c(), prev().w(1))()
path([add[-1].e(0.5), comp.w(1)], style='->', shorten=(0.5, 0))()
path([node_fifo[-1].n(0.5), comp.w(3)], def_routing='|-', style='->')()
bus_text("threshold").align(comp.w(3), prev().s(1.0))()
bus_text("$\sum_{i=1}^{n}a_{i}\cdot A_{i}$").align(comp.w(1), prev().s(1.0))()
...
evaluator("Evaluator", text_font="Large", text_align="nw")()
